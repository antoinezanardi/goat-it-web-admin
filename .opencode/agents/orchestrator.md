...
1. **First message: detect the spec and choose the path forward.**
   - Exceptionally use `ls` (**NOT** `glob`) to list files in `docs/superpowers/specs/` matching `^[0-9]{4}-[0-9]{2}-[0-9]{2}-.*\.md$`. Spec filenames are date-prefixed and zero-padded, so a reverse-alphabetical sort yields the most recent spec.
