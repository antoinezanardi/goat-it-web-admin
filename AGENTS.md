...
- `app/`              – Nuxt application source (SPA — `ssr: false`)
  - `components/`     – `layouts/`, `shared/`, `domain/` sub-dirs; PascalCase `.vue` files
  ...
  - `pages/`, `plugins/`, `templates/`, `utils/`, `assets/`
...
- SPA-specific patterns (admin is `ssr: false`):
  - Pages use `definePageMeta` with `titleKey`, `icon`, `order` for sidebar navigation. Each page has a `.constants.ts` exporting `PAGE_TITLE_KEY`, `PAGE_ICON`, `PAGE_ORDER`.
  - Data is loaded on first visit via `void callOnce(store.fetchAndStoreXxx())` in page components.
  - Form modals follow portal/emit pattern: table emits `@start-create`/`@start-edit`, page handles modal open/close/submit.
  - SPA loading template at `app/templates/spa-loading-template.html`.
...
- `@goat-it/schemas` package (version in `package.json`):
  ...
  - Sub-paths used: `@goat-it/schemas/question`, `@goat-it/schemas/question-theme`, `@goat-it/schemas/shared/error`, `@goat-it/schemas/shared/locale`, `@goat-it/schemas/shared/constants`.
  - Used in server handlers for response/query/body validation (`parse()` before mapping to domain types) and in app code for type-only imports (`import type` for DTO types, domain constant arrays).
...
- Wing: `goat_it_web_admin` | Rooms: `app`, `testing`, `documentation`, `backend`, `configuration`, `scripts`, `general`, `docker`, `design`, `modules`, `shared`
- For codebase context (architecture, patterns, past decisions): `mcp mempalace_search "query" --wing goat_it_web_admin`
