...
import { archiveQuestionHandler } from "#server/api/goat-it-api/questions/handlers/archive/[id].archive.post.handler";
...
describe("Server Goat It API Questions Archive Handler", () => {
  const fakeId = "abc123";
  ...
  describe(archiveQuestionHandler, () => {
    it("should get router param id from event when called.", async() => {
      await archiveQuestionHandler(mockedEvent);
    ...
    it("should fetch with POST method and archive endpoint when called.", async() => {
      ...
      expect($fetch).toHaveBeenCalledExactlyOnceWith(`${expectedEndpoint}/archive`, {
    ...
    it("should return mapped question when called.", async() => {
      ...
      const result = await archiveQuestionHandler(mockedEvent);
    ...
    it("should throw when getRequiredRouterParam throws for missing id.", async() => {
      ...
      await expect(archiveQuestionHandler(mockedEvent)).rejects.toThrow("Question id is required");
    });
  });
});