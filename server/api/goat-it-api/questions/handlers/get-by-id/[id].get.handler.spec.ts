...
describe("Server Goat It API Questions Get By Id Handler", () => {
  const fakeId = "abc123";
  ...
  describe(getQuestionByIdHandler, () => {
    it("should get router param id from event when called.", async() => {
      await getQuestionByIdHandler(mockedEvent);
    ...
    it("should fetch question from goat it api with correct endpoint and fetch options when called.", async() => {
    ...
    it("should return mapped question when called.", async() => {
      ...
      const result = await getQuestionByIdHandler(mockedEvent);
    ...
    it("should throw when getRequiredRouterParam throws for missing id.", async() => {
      ...
      await expect(getQuestionByIdHandler(mockedEvent)).rejects.toThrow("Question id is required");
    });
  });
});